...
do $$
declare req public.context_requests; module jsonb; claim jsonb; saved jsonb; kind text; topic text; evidence text;
begin
 ...
 for topic,evidence in select * from (values ('musicbrainz_recordings','observation'),('mlc_recordings','observation'),('mlc_works','observation'),('chartmetric_candidates','observation'),('social_context','observation'),('catalog_valuation','estimate'),('lyrics','interpretation')) t(topic,evidence) loop
  ...
  saved:=public.complete_context_enrichment(req.owner_id,req.id,(claim->>'attemptId')::uuid,jsonb_build_object('content',jsonb_build_object('fixture',true),'coverage','unknown','trace',jsonb_build_object('fixture',true),'costUsd',null,'costStatus','unknown'));
