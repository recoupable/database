-- Add typed provider evidence while preserving owner/subject checks, source lineage and conservative retry handling.
-- Existing modules without evidenceKind retain interpretation. No API dispatch or paid calls are enabled.
begin;
...
begin
 ...
 if p_module->>'fingerprint' !~ '^[a-f0-9]{64}$' or p_module->>'topic' not in ('catalog_metadata','lyrics','song_summary','artwork_branding','artist_research','musicbrainz_recordings','mlc_recordings','mlc_works','mlc_work_candidates','chartmetric_candidates','songstats_context','social_context','catalog_valuation') then raise exception 'Invalid enrichment module'; end if;
 ...
 if p_module->>'topic' in ('musicbrainz_recordings','mlc_recordings','mlc_works','mlc_work_candidates','chartmetric_candidates','songstats_context','social_context') and coalesce(p_module->>'evidenceKind','')<>'observation' then raise exception 'Provider lookup requires observation evidence'; end if;
...
returns jsonb language plpgsql set search_path='' as $$
declare a public.context_attempts; d public.context_documents; result uuid; source uuid; version uuid; item jsonb; hash text; evidence_sources jsonb;
begin
