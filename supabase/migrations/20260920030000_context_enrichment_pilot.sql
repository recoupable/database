-- Independent enrichment attempts. This does not authorize customer spend or
-- enable paid HTTP ingestion; billing reservations remain a separate gate.
begin;
alter table public.context_attempts add column input jsonb, add column error text;
create index context_attempts_fingerprint on public.context_attempts(owner_id,(input->>'fingerprint'));

create function public.claim_context_enrichment(p_owner uuid,p_request uuid,p_module jsonb)
...
begin
 ...
 if p_module->>'fingerprint' !~ '^[a-f0-9]{64}$' or p_module->>'topic' not in ('catalog_metadata','lyrics','song_summary','artwork_branding','artist_research') then raise exception 'Invalid enrichment module'; end if;
...
create function public.complete_context_enrichment(p_owner uuid,p_request uuid,p_attempt uuid,p_result jsonb)
returns jsonb language plpgsql set search_path='' as $$
declare a public.context_attempts; d public.context_documents; result uuid; source uuid; version uuid; item jsonb; hash text;
begin
 ...
 if p_result->>'coverage' not in ('full','partial','unknown') or p_result->'content' is null or jsonb_array_length(a.input->'sources')=0 then raise exception 'Missing evidence'; end if;
 ...
 values(p_owner,p_attempt,d.subject_id,d.topic,a.input->>'fingerprint','accepted','interpretation',p_result->'trace',p_result->'content',jsonb_build_object('extent',p_result->>'coverage','recipe',a.recipe_version)) returning id into result;
 for item in select value from jsonb_array_elements(a.input->'sources') loop
...
create function public.fail_context_enrichment(p_owner uuid,p_attempt uuid)
returns boolean language plpgsql set search_path='' as $$
declare n integer;
begin
 update public.context_attempts set status='unknown',finished_at=now(),error='Enrichment did not finish saving. Inspect the provider trace and reconcile before retrying.' where owner_id=p_owner and id=p_attempt and status='running';
 get diagnostics n=row_count;return n=1;
end $$;

do $$ declare f text; begin
 foreach f in array array['claim_context_enrichment(uuid,uuid,jsonb)','complete_context_enrichment(uuid,uuid,uuid,jsonb)','fail_context_enrichment(uuid,uuid)'] loop
