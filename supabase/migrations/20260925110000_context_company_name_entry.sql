-- A submitted business name is workspace evidence, not an organization membership or verified company.
begin;
...
alter table public.context_resources add constraint context_resources_resource_kind_check
 check(resource_kind in ('artist','track','release','video','catalog','songwriter_input','company_input'));
...
alter table public.context_subjects add constraint context_subjects_kind_check
 check(kind in ('artist','recording','release','video','catalog','songwriter','company'));
...
alter table public.context_subjects add constraint context_subjects_check check(
 ...
 (kind in ('release','video','songwriter','company') and resource_id is not null and artist_id is null and song_isrc is null and catalog_id is null) or
 (kind='catalog' and catalog_id is not null and artist_id is null and song_isrc is null and resource_id is null)
);

create function public.create_context_company_name_request(
...
begin
 if p_name is null or p_name ~ '[[:cntrl:]]' then raise exception 'Invalid company name'; end if;
 normalized:=pg_catalog.btrim(pg_catalog.regexp_replace(p_name,'[[:space:]]+',' ','g'));
 if length(normalized) not between 2 and 200 then raise exception 'Invalid company name'; end if;
 input_hash:=encode(sha256(convert_to('company-name-v1:'||lower(normalized),'UTF8')),'hex');
 opaque:=encode(sha256(convert_to('company-input-v1:'||p_owner::text||':'||p_key,'UTF8')),'hex');
 uri:='urn:recoup:company-input:'||opaque;
 insert into public.context_resources(provider,resource_kind,provider_id,canonical_url)
 values('recoup','company_input',opaque,uri)
 ...
 values(p_owner,p_actor,resource,p_key,input_hash,jsonb_build_object('kind','company','name',normalized,'identityConfirmed',false))
 ...
 insert into public.context_subjects(kind,resource_id) values('company',resource)
 ...
 insert into public.context_documents(owner_id,subject_id,topic) values(p_owner,subject,'company_input') on conflict do nothing;
 select * into strict document from public.context_documents
 where owner_id=p_owner and subject_id=subject and topic='company_input' for update;
 ...
 values(p_owner,req.id,'company_input:'||subject,1,'succeeded','user_submission','company-name-v1','1',now(),now(),0,'confirmed')
 ...
 values(p_owner,attempt_key,subject,'company_input',snapshot_hash,'accepted','customer_assertion',snapshot,snapshot,
 ...
 then raise exception 'Company input acceptance conflict'; end if;
 update public.context_requests set status='partial',
  output=jsonb_build_object('subjectIds',jsonb_build_array(subject),'gaps',jsonb_build_array('Company identity and workspace relationship not confirmed')),
  updated_at=now() where id=req.id returning * into req;
 return to_jsonb(req)-'claim_token';
end $$;
revoke all on function public.create_context_company_name_request(uuid,uuid,text,text) from public,anon,authenticated;
grant execute on function public.create_context_company_name_request(uuid,uuid,text,text) to service_role;

create function public.list_context_company_request_target(p_owner uuid,p_request uuid)
returns jsonb language plpgsql set search_path='' as $$
...
begin
 ...
 if req.status not in ('partial','completed') or req.input->>'kind'<>'company'
  or jsonb_typeof(req.output->'subjectIds') is distinct from 'array'
  or jsonb_array_length(req.output->'subjectIds')<>1
 then raise exception 'Company request is not ready'; end if;
 ...
 join public.context_resources r on r.id=s.resource_id and r.id=req.resource_id
  and r.provider='recoup' and r.resource_kind='company_input'
 join public.context_documents d on d.owner_id=p_owner and d.subject_id=s.id and d.topic='company_input'
 ...
 where s.kind='company' and req.output->'subjectIds' ? s.id::text
 ...
 return jsonb_build_object('subjectId',subject.id,'kind','company','identityConfirmed',false,
  'availableFields',jsonb_build_array('submitted_name'),'reusableModules','[]'::jsonb);
end $$;
revoke all on function public.list_context_company_request_target(uuid,uuid) from public,anon,authenticated;
grant execute on function public.list_context_company_request_target(uuid,uuid) to service_role;
commit;
